...
// Tipos de venta
const TIPOS_VENTA = {
    pieza: { id: 2, label: 'Piezas', descuento: 0 },
    gramaje: { id: 1, label: 'Gramaje', descuento: 0 },
    medio_kilo: { id: 3, label: 'Paquete 700 g', descuento: 0.05 },
    kilo: { id: 4, label: 'Paquete Kilo', descuento: 0.10 }
};

function calcCantidadGalletas(cantidad, tipo_venta, gramosPorPieza) {
    ...
    switch (tipo_venta) {
        ...
        case 'gramaje':
            ...
            break;
        case 'medio_kilo':
            cantidadGalletas = Math.floor((cantidad * 700) / gramosPorPieza);
            break;
        case 'kilo':
...
function agregarAlCarritoDesdeGalleta(id_producto, nombre, precio_unitario, gramos_galleta, boton) {
    ...
    const gramosPorPieza = gramos_galleta;
    ...
    switch (tipo_venta) {
        ...
        case 'medio_kilo':
            ...
            precioUnitarioAjustado = precio_unitario * Math.floor(700 / gramosPorPieza) * (1 - descuento);
            subtotal = cantidad * precioUnitarioAjustado;
            break;
        case 'kilo':
            // Siguiendo la lógica de registro-ventas.js para kilo
            precioUnitarioAjustado = precio_unitario * Math.floor(1000 / gramosPorPieza) * (1 - descuento);
            subtotal = cantidad * precioUnitarioAjustado;
            break;
        default:
            precioUnitarioAjustado = precio_unitario;
    ...
    if (index !== -1) {
        ...
        carrito[index].cantidad_total += cantidadTotal;
    ...
    let mensaje = `Agregaste ${cantidad} ${tipo_venta === 'pieza' ? 'pieza(s)' :
        tipo_venta === 'gramaje' ? 'gramo(s)' :
            tipo_venta === 'medio_kilo' ? 'Paquete 700 g' : 'Paquete Kg'} de galletas`;
